...
for i in [1, 2, 4, 8, 16]
    ...
    if i == 1
        display(plot([1 / i], [mean(abun[:, :, 1500:end, :])],
                     seriestype = :scatter, label = "", ylim = (0, 5 * 10^4),
                     xlim = (0, 1.1), grid = false,
                     ylab = "Average abundance at eqm",
                     xlab = "1 / number of species"))
    else
        display(plot!([1 / i], [mean(abun[:, :, 1500:end, :])],
...
heatmap(mapslices(x -> sum(x .> 0), abun, dims = 1)[1, :, :, 1],
        xlab = "Time (months)", ylab = "Space")
...
for i in 1:10:200
    ...
    if i == 1
        display(heatmap(b, layout = (2, 1), clim = (1, 4),
                        title = "Subcommunity β̄"))
    else
        display(heatmap!(b, subplot = 1, clim = (1, 4)))
    end
    display(plot!([i], B, seriestype = :scatter, subplot = 2, xlim = (0, 200),
                  ylim = (1, 4), grid = false, label = "",
                  title = "Metacommunity β̄"))
end
...
for i in 1:10:200
    ...
    if i == 1
        display(heatmap(b, layout = (2, 1), clim = (0.2, 0.6),
                        title = "Subcommunity ρ̄"))
    else
        display(heatmap!(b, subplot = 1, clim = (0.2, 0.6)))
    end
    display(plot!([i], B, seriestype = :scatter, subplot = 2, xlim = (0, 200),
                  ylim = (0, 1), grid = false, label = "",
                  title = "Metacommunity ρ̄"))
end
...
for i in 1:20:600
    ...
    if i == 1
        display(heatmap(b, layout = (2, 1), clim = (1, 1000),
                        title = "Subcommunity ᾱ"))
    else
        display(heatmap!(b, subplot = 1, clim = (1, 1000)))
    end
    display(plot!([i], B, seriestype = :scatter, subplot = 2, xlim = (0, 600),
                  ylim = (1, 5000), grid = false, label = "", color = :1,
                  title = "Metacommunity γ"))
end
...
for i in 1:24
    ...
    if i == 1
        display(heatmap(b, layout = (2, 1), clim = (40000, 50000),
                        title = "Subcommunity ᾱ"))
    else
        display(heatmap!(b, subplot = 1, clim = (40000, 50000)))
    end
    display(plot!([i], B, seriestype = :scatter, subplot = 2, xlim = (0, 24),
                  ylim = (199500, 200100), grid = false, label = "", color = :1,
                  title = "Metacommunity γ"))
end
...
for i in 1:50:1200
    ...
    B = metadiv(γ(meta), 0)[:diversity]
    if i == 1
        display(heatmap(b, layout = (2, 1), clim = (1000, 3000),
                        title = "Subcommunity ᾱ"))
    else
        display(heatmap!(b, subplot = 1, clim = (1000, 3000)))
    end
    display(plot!([i], B, seriestype = :scatter, subplot = 2, xlim = (0, 1200),
                  ylim = (2800, 3000), grid = false, label = "", color = :1,
                  title = "Metacommunity γ"))
end
# Animation beta bar over time
for i in 1:50:1200
    ...
    B = metadiv(β̄(meta), 1)[:diversity]
    if i == 1
        display(heatmap(b, layout = (2, 1), clim = (1, 3),
                        title = "Subcommunity β̄"))
    else
        display(heatmap!(b, subplot = 1, clim = (1, 3)))
    end
    display(plot!([i], B, seriestype = :scatter, subplot = 2, xlim = (0, 1200),
                  ylim = (1, 3), grid = false, label = "",
                  title = "Metacommunity β̄"))
end
...
grd = (300, 150);
req = 10.0kJ;
individuals = 5_000_000;
area = 300_000_000.0 * km^2;
totalK = 1000.0kJ / km^2;
