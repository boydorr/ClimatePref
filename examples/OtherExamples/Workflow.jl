...
t = open(io -> parsenewick(io,
                           PolytomousTree{LeafInfo, IndexedTables.NextTable}),
         "data/Qian2016.tree")
tree_names = collect(nodenamefilter(isleaf, t))
sol_names = filter(name -> contains(name, r"Solanum"),
                   collect(nodenamefilter(isleaf, t)))
...
map(cross_species, cross_species_names) do spp_name, tree_name
    ...
    return setnoderecord!(t, tree_name, clean_dat)
end
...
soltraits = map(x -> na_mean(select(getnoderecord(t, x), :tavg)),
                cross_species_names)
...
datavals = find(map(x -> typeof(x) <: DataValues.DataValue,
                    select(gbif, :na_mean)))
...
newtab = pushcol(newtab, :species_name,
                 join.(split.(select(newtab, :species), " "), "_"))
...
for i in 1:(nnodes - 1)
    d[((i - 1) * (nnodes - 1)):((i) * (nnodes - 1))] = map(x -> distance(t,
                                                                         tree_names[i],
                                                                         x),
                                                           tree_names[(i + 1):end])
    ta[((i - 1) * (nnodes - 1) + i):((i) * (nnodes - 1))] = abs(traits[i, 1] .-
                                                                traits[(i + 1):end,
                                                                       1])
    n = n + 1
end
for i in 1:(nnodes - 1)
    if i == 1
        ...
        d = [d;
             map(x -> distance(t, tree_names[i], x), tree_names[(i + 1):end])]
