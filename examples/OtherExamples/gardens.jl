...
gridsize = 0.02°
refarray = AxisArray(Array{Int64, 2}(18001, 9001),
...
function gardenmask!(occ::JuliaDB.Table, gard::JuliaDB.Table, masksize::Float64)
    ...
    coords2 = hcat(select(sol, :decimallatitude),
                   select(sol, :decimallongitude))
    ...
    return rem = vcat(removals[!isempty.(removals)]...)
end
function gardenmask(occ::IndexedTables.NextTable, gard::IndexedTables.NextTable,
                    masksize::Float64)
    coords = hcat(select(gard, :Latitude), select(gard, :Longitude))
    coords2 = hcat(select(occ, :decimallatitude),
