...
function Columns(cols::AbstractArray...;
                 names::Union{Vector, Tuple{Vararg{Any}}, Void} = nothing)
...
for i in 2:length(worldclim_collect)
    values = worldclim_collect[i].array[1:end]
    worldclim_tab = table(repmat(newx, 12), repmat(newy, 12), values, months,
                          names = [
                              :x,
                              :y,
                              Symbol(worldclim_names[i + 1]),
                              :month
                          ])
    if (i == 1)
    ...
    JuliaDB.save(worldclim_tab,
                 string("data/Worldclim/Monthly/", worldclim_names[i + 1]))
end
...
sol = pushcol(sol, :refval,
              extractvalues(coords_sol[:, 2] * °, coords_sol[:, 1] * °, ref))
...
for i in 1:7
    ...
    sol = join(sol, worldclim_dat, how = :left, lkey = :refval, rkey = :refval,
               rselect = Symbol(worldclim_names[i + 1]))
end
...
x = collect(axes(bio.array[:, :, i], 1).val)
y = collect(axes(bio.array[:, :, i], 2).val)
expandedxy = collect(product(x, y))
newx = map(x -> x[1], expandedxy)
newy = map(x -> x[2], expandedxy)
...
for i in 1:length(bioclim_names)
    if (i == 1)
        bioclim_tab = table(newx, newy, bio.array[:, :, i][1:end] * units[i],
                            names = [:x, :y, Symbol(bioclim_names[i])])
    else
        bioclim_tab = pushcol(bioclim_tab, Symbol(bioclim_names[i]),
                              bio.array[:, :, i][1:end] * units[i])
    end
end
