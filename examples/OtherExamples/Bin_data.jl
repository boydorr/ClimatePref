...
splits = Dict(zip(worldclim,
                  [collect(0:50:3000), collect(0:500:70000),
...
j = 1
for j in 2:7
    @everywhere func(y) = fit(Histogram, ustrip.(y), splits[worldclim[j]],
                              closed = :right).weights
    bins = @sync @parallel (merge) for i in eachindex(genera)
        genus = JuliaDB.load(string("Genera/Worldclim/Monthly/", genera[i]))
        spp = unique(select(genus, :species))
        spp = spp[spp .!= ""]
        if length(spp) == 0
            bin = AxisArray(Array{Int64, 2}(0,
                                            length(splits[worldclim[j]][2:end])),
                            Axis{:Species}(spp),
                            Axis{:Bins}(splits[worldclim[j]][2:end]))
        else
            res = groupby(:func => func, genus, :species, select = worldclim[j])
...
R"""
dat = data.frame(vec=vec, temps=temps)
library(ggplot2)
library(cowplot)
...
species = @sync @parallel (merge) for i in eachindex(genera)
    genus = JuliaDB.load(string("Genera/Worldclim/",
                                split(genera[i], ".csv")[1]))
    if length(genus) == 0
