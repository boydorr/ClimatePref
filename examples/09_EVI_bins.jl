...
cera_counts = @groupby cera_simple :refval {tmin = bin(uconvert.(K, :tmin),
                                                       mins[1], maxs[1]),
                                            tmax = bin(uconvert.(K, :tmax),
                                                       mins[2], maxs[2]),
                                            tmean = bin(uconvert.(K, :tmean),
                                                        mins[3], maxs[3]),
                                            trng = bin(uconvert.(K, :trng),
                                                       mins[4], maxs[4]),
                                            stl1 = bin(uconvert.(K, :stl1mean),
                                                       mins[5], maxs[5]),
                                            stl2 = bin(uconvert.(K, :stl2mean),
                                                       mins[6], maxs[6]),
                                            stl3 = bin(uconvert.(K, :stl3mean),
                                                       mins[7], maxs[7]),
                                            stl4 = bin(uconvert.(K, :stl4mean),
                                                       mins[8], maxs[8]),
                                            swvl1 = bin(:swvl1mean, mins[9],
                                                        maxs[9]),
                                            swvl2 = bin(:swvl2mean, mins[10],
                                                        maxs[10]),
                                            swvl3 = bin(:swvl3mean, mins[11],
                                                        maxs[11]),
                                            swvl4 = bin(:swvl4mean, mins[12],
                                                        maxs[12]),
                                            ssr = bin(:ssrmean, mins[13],
                                                      maxs[13]),
                                            tp = bin(:tpmean, mins[14],
                                                     maxs[14])}
...
cera_counts = @transform cera_counts {tmin = :tmin .* :evi,
                                      tmax = :tmax .* :evi,
                                      tmean = :tmean .* :evi,
                                      trng = :trng .* :evi,
                                      stl1 = :stl1 .* :evi,
...
total_evi_counts = [count_convert(cera_counts, i)
                    for i in [
                            :tmin,
                            :tmax,
                            :tmean,
...
evi_tab = JuliaDB.load("EVI")
evi_cont = join(evi_tab, continents, how = :inner, lkey = :refval,
                rkey = :refval)
evi_cont = filter(e -> !ismissing(e.continent), evi_cont)
continents = distribute(continents, 1)
cera_cont = join(cera_simple, continents, how = :inner, lkey = :refval,
                 rkey = :refval)
cera_cont = filter(e -> !ismissing(e.continent), cera_cont)

function binContinents(cera::JuliaDB.DIndexedTable, evi::JuliaDB.IndexedTable,
                       continent::Int64, mins, maxs)
    ...
    cera_counts = @groupby cera :refval {tmin = bin(uconvert.(K, :tmin),
                                         ...
                                         swvl4 = bin(:swvl4mean, mins[12],
                                                     maxs[12]),
                                         ssr = bin(:ssrmean, mins[13], maxs[13]),
                                         tp = bin(:tpmean, mins[14], maxs[14])}
...
function mapContinents!(cera::JuliaDB.DIndexedTable, evi::JuliaDB.IndexedTable,
                        continents::Vector{Int64}, mins, maxs)
