...
function fitAdjust!(gbif::JuliaDB.DIndexedTable, tree::HybridNetwork,
                    continents::Vector{Int64}, spp_dict::Dict,
                    top_common_names::Vector{String}, mins, maxs,
                    filename::String; climate::Bool = false, raw::Bool = false)
    for c in continents[1:end]
        ...
        dat = adjustData(gbif_fil, spp_dict, c, top_common_names, adjustment,
                         mins, maxs)
...
fitAdjust!(gbif_fil, tree, collect(1:6), spp_dict, top_common_names, mins, maxs,
           "Continent_dat_raw", raw = true)
fitAdjust!(gbif_fil, tree, collect(1:6), spp_dict, top_common_names, mins, maxs,
           "Continent_dat_adjust")
fitAdjust!(gbif_fil, tree, collect(1:6), spp_dict, top_common_names, mins, maxs,
           "Continent_dat_adjust2", climate = true, raw = false)
...
heatmap(seriescolor = :Blues, size = (1000, 500), guidefontsize = 12,
        tickfontsize = 12, xrotation = 90, clim = (0, 1), colorbar_title = "λ",
        layout = (@layout [a b c; d e f]), link = :both, margin = 5 * Plots.mm,
        titlefontsize = 14)
continents = [
    "Africa",
    "South America",
...
for i in 1:6
    ...
    display(heatmap!(y, x, transpose(lambdas[subset, :]),
                     seriescolor = cgrad([:red, :white, :blue],
                                         [0, mean_lambda, 1]),
                     colorbar = :legend, guidefontsize = 12, tickfontsize = 12,
                     xrotation = 90, clim = (0, 1), colorbar_title = "λ",
                     subplot = i, title = continents[i], legend = legend,
                     xaxis = xax, yaxis = yax))
end
...
dat = collect(dat)
dat = @transform dat {tmin = ustrip.(uconvert.(°C, :tmin)),
                      tmax = ustrip.(uconvert.(°C, :tmax)),
                      tmean = ustrip.(uconvert.(°C, :tmean)),
                      trng = ustrip.(uconvert.(°C, :trng)),
                      stl1 = ustrip.(uconvert.(°C, :stl1)),
                      stl2 = ustrip.(uconvert.(°C, :stl2)),
                      stl3 = ustrip.(uconvert.(°C, :stl3)),
                      stl4 = ustrip.(uconvert.(°C, :stl4)),
                      swvl1 = ustrip.(:swvl1), swvl2 = ustrip.(:swvl2),
                      swvl3 = ustrip.(:swvl3), swvl4 = ustrip.(:swvl4),
...
names = DataFrame(name = names[indexin(continent[:tipNames], names)],
                  cont = continent[:c])
