...
total_cera_counts = [bin(cera_simple, vars[i], mins[i], maxs[i])
...
function binContinents(cera::JuliaDB.DIndexedTable, continent::Int64, mins,
                       maxs, vars)
    cera = filter(c -> c.continent == continent, cera)
    total_cera_counts = [bin(cera, vars[i], mins[i], maxs[i])
                         for i in eachindex(vars)]
    return total_cera_counts = hcat(total_cera_counts...)
end

function mapContinents!(cera::JuliaDB.DIndexedTable, continents::Vector{Int64},
                        mins, maxs, vars)
