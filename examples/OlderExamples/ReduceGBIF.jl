...
if i == "Aa" tab = groupreduce(na_mean, genus, :species, select=:tavg)
...
@everywhere bioclim_names = ["annual_tavg", "mean_diurnal_range", "isothermality",
...
tab  = @sync @parallel (merge) for i in searchdir(dir, "")
    genus = load(string(dir,i))
    if length(genus) > 0
    red_genus = groupby(:annual_tavg => na_mean, genus, :species, select=:annual_tavg)
    for j in 2:19
