...
raw_traits_filter = filter(p -> p.tipNames in join.(split.(top_common_names,
                                                           " "), "_"),
                           raw_traits)
...
scatter(old ./ K, new, grid = false, xlabel = "Raw minimum temperature (K)",
        ylabel = "Reconstructed minimum temperature (K)", ms = 2, label = "",
        markercolour = :grey, markerstrokecolour = false)
...
scatter(old ./ K, new, grid = false, xlabel = "Raw maximum temperature (K)",
        ylabel = "Reconstructed maximum temperature (K)", ms = 2, label = "",
        markercolour = :grey, markerstrokecolour = false)
...
nodeindices = map(a -> PhyloNetworks.getIndexNode(a, tree),
                  ancInt[:, :nodeNumber])
...
sort!(tree2)
plot(tree2, line_z = "tmin", lw = 1, layout = (@layout [a b c]),
     size = (1600, 800),
     title = "A", title_loc = :left,
     colorbar_title = "Minimum temperature (°C)", guidefontsize = 16,
     titlefontsize = 18,
     tickfontsize = 14, linecolor = :matter)
plot!(tree2, line_z = "tp", lw = 1, subplot = 2, linecolor = :haline,
      size = (1600, 800),
      title = "B", title_loc = :left,
      colorbar_title = "Total precipitation (mm)", guidefontsize = 16,
      titlefontsize = 18,
      tickfontsize = 14)
plot!(tree2, line_z = "ssr", lw = 1, subplot = 3, linecolor = :cividis,
      size = (1600, 800),
      title = "C", title_loc = :left,
      colorbar_title = "Solar radiation (kJ/m²)", guidefontsize = 16,
      titlefontsize = 18,
      tickfontsize = 14)
