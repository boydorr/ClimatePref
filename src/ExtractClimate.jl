...
function checkbounds(x::Vector{typeof(1.0°)}, y::Vector{typeof(1.0°)})
    ...
    return -90.0 .<= x .<= 90.0 || error("Y coordinate is out of bounds")
end
...
"""
function extractvalues(tab::Union{JuliaDB.IndexedTable, JuliaDB.DIndexedTable},
                       era::ERA, varname::Symbol)
    vals = map(t -> extractvalues(t.decimallatitude * °, t.decimallongitude * °,
                                  t.year, era), tab)
